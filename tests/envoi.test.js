...
const DEVIS = {
  ...
  'type-evenement': 'Anniversaire', formule: 'Micro-scénographie & Biscuits — dès 650 CHF',
...
(async () => {
  ...
  await cas('les informations reprises depuis une formule sont transmises', () => {
    ...
    const lignes = fonction.construireCorps('devis', {
      prenom: 'Camille', formule: 'Micro-scénographie & Biscuits — dès 650 CHF',
      'type-evenement': 'Anniversaire', 'theme-anniversaire': 'Licorne arc-en-ciel'
    });
    const dico = Object.fromEntries(lignes);
    assert.strictEqual(dico['Formule souhaitée'], 'Micro-scénographie & Biscuits — dès 650 CHF');
